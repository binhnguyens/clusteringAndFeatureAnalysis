...
def kmeans_cluster (gps_time, gps_lat, gps_lon, decision):
    ...
    # wcss is the distance from centroid - similar to elbow method in clusters
    wcss = normalization1 (wcss)
    der_wcss = (diff(wcss))

    # Arbitrary value that if the wcss difference is less than 0.05
    # Then the k is optimal 
    ...
    # Plotting dec 2 - The wcss is plotted 
    elif (decision ==2):
    ...
    # Normalization 
    # Total time during each cluters in seconds/Total time in study in days
        
    # normilization is to find the number of days 
    normalization = (gps_time[len(gps_time)-1]-gps_time[0])/86400
    ...
    # c1c2c3 are the different time in clusters
    # ctime is the total amount of time recorded in the 60 days... e.g 0.45
    # n_clust is number of clusters
...
def total_distance (gps_lat,gps_lon,gps_time):
    ...
    n_days = ceil((gps_time[len(gps_time)-1]-gps_time[0])/86400)
    norm_tot_dist = tot_dist/n_days

    return (norm_tot_dist)

# This fct used for Activity and Audio
def daily_analysis_1 (data,choice):
    
    #### Coding to seperate the information to days
    new_data = data['timestamp'] - min(data['timestamp'])
    # Assuming 86400 seconds is a day
    
    c = 0
    samples_per_day1 = 0
    num_days = int(ceil (np.max(new_data)/86400))
    pt1 = np.zeros(num_days,dtype=int) # Stationary or Silence
    pt2 = np.zeros(num_days,dtype=int) # Walking or Voice
    pt3 = np.zeros(num_days,dtype=int) # Running or Noise
    
    if (choice == 0):
        col_name = ' activity inference'
    elif(choice == 1):
        col_name = ' audio inference'
    
    for i in range (1,num_days):
        
        hold1 = new_data [(new_data < (86400 * i))]
        hold2 = hold1 [(hold1 > (86400 * (i-1)))]
        
        
        samples_per_day2 = len (hold2)

        focus = data [col_name][samples_per_day1:samples_per_day1+samples_per_day2]
        pt1 [c] = len (focus [focus == 0])
        pt2 [c] = len (focus [focus == 1])
        pt3 [c] = len (focus [focus == 2])        
    ...
    return (np.var (pt1),np.var (pt2),np.var (pt3),pt1,pt2,pt3)


# This fct used for Dark, Conversation and Lock
def daily_analysis_2 (data, choice):
    ...
    #### Coding to seperate the information to days
    if (choice==0):
        new_data = data['start_timestamp'] - min(data['start_timestamp'])
        
    elif (choice ==1):
        new_data = data['start'] - min(data['start'])

    # Assuming 86400 seconds is a day
    num_days = int(ceil (np.max(new_data)/86400))

    dur = np.zeros(num_days,dtype=int)
    num = np.zeros(num_days,dtype=int)
    
    # Focusses on 1 day
    # Uses fct time difference to sum all of the conversation times in that day

    for i in range (1,num_days):
        
        # Creates the array for one day 
        hold1 = new_data [(new_data < (86400 * i))]
        hold2 = hold1 [(hold1 > (86400 * (i-1)))]
        samples_per_day2 = len (hold2)

        focus = data [samples_per_day1:samples_per_day1+samples_per_day2]
    ...
    # dur is the array of durations per day
    # n_convo is the number of convos per day    
    return (dur,num)
